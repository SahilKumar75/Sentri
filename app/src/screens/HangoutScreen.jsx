...
import { buildSeedActivity, buildSeedChat, buildSeedParticipants, defaultMeetingSettings, } from '../lib/hangout-meeting';
import { createRoom, extractRoomCode, getRoom, joinRoom, listRooms, } from '../lib/hangout-api';
const friends = [
...
export default function HangoutScreen({ onOpenDrawer, avatarLabel, sessionToken, userName, incomingRoomCode, onConsumeIncomingRoomCode, onMeetingModeChange, }) {
    ...
    const [meetingSettings, setMeetingSettings] = useState(defaultMeetingSettings);
    ...
    const tileParticipants = useMemo(() => meetingSettings.layout === 'grid'
        ? meetingParticipants
        : meetingParticipants.filter((participant) => participant.id !== stageParticipant?.id), [meetingParticipants, meetingSettings.layout, stageParticipant]);
    ...
    async function handleOpenRoom(roomCode) {
    ...
    function muteAllGuests() {
        setMeetingParticipants((current) => current.map((participant, index) => (index === 0 ? participant : { ...participant, muted: true })));
    ...
    function sendReaction(icon, label) {
    ...
    function renderLobby() {
        return (<ScrollView style={styles.lobbyScreen} contentContainerStyle={styles.lobbyContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.actionRow}>
            <Pressable style={[styles.actionButton, styles.actionButtonFilled, loadingAction !== 'idle' && styles.disabledButton]} onPress={() => void handleCreateRoom()} disabled={loadingAction !== 'idle'}>
        ...
        <View style={styles.section}>
          ...
          {friends.map((friend) => (<View key={friend.name} style={styles.friendRow}>
              ...
              <Pressable style={[styles.inviteButton, !activeRoom && styles.disabledButton]} onPress={() => void handleShareRoom(friend.name)} disabled={!activeRoom}>
    ...
    function renderMeeting() {
        return (<View style={styles.meetingScreen}>
        ...
        <View style={styles.meetingStage}>
          <View style={styles.stageCard}>
            <View style={styles.stageTopRow}>
              ...
              {recordingOn ? (<View style={styles.recordingChip}>
                  <View style={styles.recordingDot}/>
                  <Text style={styles.recordingText}>Recording</Text>
                </View>) : null}
            </View>

            {shareScreenOn ? (<View style={styles.shareStage}>
                ...
                <View style={styles.tileFooter}>
                  ...
                  <View style={styles.tileFooterIcons}>
                    {stageParticipant?.handRaised ? (<Ionicons name="hand-left" size={15} color="#FFFFFF"/>) : null}
                    <Ionicons name={stageParticipant?.muted ? 'mic-off' : 'mic'} size={15} color="#FFFFFF"/>
                  </View>
                </View>
              </View>)}

            {captionsOn ? (<View style={styles.captionBubble}>
          ...
          <View style={styles.tileStrip}>
            {tileParticipants.map((participant) => (<Pressable key={participant.id} style={[
        ...
        <View style={styles.controlDock}>
          ...
          <View style={styles.controlRow}>
            <MeetingControlButton label={micOn ? 'Mic on' : 'Mic off'} icon={micOn ? 'mic' : 'mic-off'} active={micOn} onPress={toggleMic}/>
            <MeetingControlButton label={cameraOn ? 'Video on' : 'Video off'} icon={cameraOn ? 'videocam' : 'videocam-off'} active={cameraOn} onPress={toggleCamera}/>
            <MeetingControlButton label={shareScreenOn ? 'Stop share' : 'Share'} icon={shareScreenOn ? 'stop-circle' : 'desktop'} active={shareScreenOn} onPress={toggleShareScreen}/>
            <MeetingControlButton label={handRaised ? 'Hand up' : 'Raise'} icon="hand-left" active={handRaised} onPress={toggleHandRaised}/>
          </View>

          <View style={styles.controlRow}>
            <MeetingControlButton label={captionsOn ? 'Captions on' : 'Captions'} icon="chatbubble-ellipses" active={captionsOn} onPress={() => setCaptionsOn((current) => !current)}/>
            <MeetingControlButton label="People" icon="people" active={meetingPanel === 'people'} onPress={() => setMeetingPanel('people')}/>
            <MeetingControlButton label="Chat" icon="chatbox" active={meetingPanel === 'chat'} onPress={() => setMeetingPanel('chat')}/>
            <MeetingControlButton label="Settings" icon="settings" active={meetingPanel === 'settings'} onPress={() => setMeetingPanel('settings')}/>
        ...
        <MeetingPanelSheet visible={meetingPanel !== 'none'} panel={meetingPanel} participants={meetingParticipants} messages={chatMessages} activityFeed={activityFeed} chatDraft={chatDraft} onChangeChatDraft={setChatDraft} onSendChat={sendChatMessage} settings={meetingSettings} onClose={() => setMeetingPanel('none')} onToggleRecording={toggleRecording} recordingOn={recordingOn} onShareRoom={() => void handleShareRoom()} onUpdateSettings={setMeetingSettings} isHost={isHost} onMuteAllGuests={muteAllGuests} onLowerAllHands={lowerAllHands}/>
      </View>);
    }
    return meetingOpen ? renderMeeting() : renderLobby();
}
function MeetingControlButton({ label, icon, active, onPress, }) {
    return (<Pressable style={[styles.controlButton, active && styles.controlButtonActive]} onPress={onPress}>
      <Ionicons name={icon} size={18} color="#FFFFFF"/>
      <Text style={styles.controlButtonText}>{label}</Text>
    </Pressable>);
}
function MeetingPanelSheet({ visible, panel, participants, messages, activityFeed, chatDraft, onChangeChatDraft, onSendChat, settings, onClose, onToggleRecording, recordingOn, onShareRoom, onUpdateSettings, isHost, onMuteAllGuests, onLowerAllHands, }) {
    return (<Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.panelBackdrop}>
        ...
        <View style={styles.panelSheet}>
          ...
          {panel === 'people' ? (<ScrollView style={styles.panelScroll} showsVerticalScrollIndicator={false}>
              {isHost ? (<View style={styles.hostToolsRow}>
                ...
                </View>) : (<View style={styles.guestNotice}>
                  <Text style={styles.guestNoticeText}>Only the host can change room-wide controls.</Text>
                </View>)}
              {participants.map((participant) => (<View key={participant.id} style={styles.panelRow}>
                  ...
                  <View style={styles.panelIcons}>
                    {participant.handRaised ? (<Ionicons name="hand-left" size={15} color={theme.colors.textSoft}/>) : null}
                    <Ionicons name={participant.muted ? 'mic-off' : 'mic'} size={15} color={theme.colors.textSoft}/>
                    <Ionicons name={participant.videoOn ? 'videocam' : 'videocam-off'} size={15} color={theme.colors.textSoft}/>
                  </View>
                </View>))}
            </ScrollView>) : null}

          {panel === 'chat' ? (<View style={styles.chatSheet}>
              {!settings.allowChat ? (<View style={styles.guestNotice}>
                  <Text style={styles.guestNoticeText}>Chat is off for this room, so new messages are blocked.</Text>
                </View>) : null}
              <ScrollView style={styles.panelScroll} showsVerticalScrollIndicator={false}>
                {messages.map((message) => (<View key={message.id} style={styles.messageCard}>
              ...
              <View style={styles.chatComposer}>
                <TextInput style={[styles.chatInput, !settings.allowChat && styles.chatInputDisabled]} value={chatDraft} onChangeText={onChangeChatDraft} placeholder={settings.allowChat ? 'Send a message' : 'Chat disabled by host'} placeholderTextColor={theme.colors.textMuted} editable={settings.allowChat}/>
                <Pressable style={[styles.chatSendButton, !settings.allowChat && styles.chatSendButtonDisabled]} onPress={onSendChat} disabled={!settings.allowChat}>
                  <Ionicons name="send" size={16} color="#FFFFFF"/>
                </Pressable>
              </View>
            </View>) : null}

          {panel === 'details' ? (<ScrollView style={styles.panelScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.detailSection}>
                ...
                {activityFeed.map((item) => (<View key={item.id} style={styles.activityRow}>
          ...
          {panel === 'settings' ? (<ScrollView style={styles.panelScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.settingsChipRow}>
                {['spotlight', 'grid'].map((layout) => {
                const active = settings.layout === layout;
                return (<Pressable key={layout} style={[styles.settingsChip, active && styles.settingsChipActive]} onPress={() => onUpdateSettings({ ...settings, layout })}>
              ...
              <View style={styles.settingsChipRow}>
                {['Speaker', 'Earpiece', 'Bluetooth'].map((output) => {
                const active = settings.audioOutput === output;
                return (<Pressable key={output} style={[styles.settingsChip, active && styles.settingsChipActive]} onPress={() => onUpdateSettings({ ...settings, audioOutput: output })}>
              ...
              <View style={styles.settingsChipRow}>
                {['Auto', '720p'].map((quality) => {
                const active = settings.videoQuality === quality;
                return (<Pressable key={quality} style={[styles.settingsChip, active && styles.settingsChipActive]} onPress={() => onUpdateSettings({ ...settings, videoQuality: quality })}>
