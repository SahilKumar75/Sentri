...
import { explainMatch, rankMyspaceItems } from '../features/myspace/retrieval-engine';
import { PERSISTENT_KEYS } from '../lib/persistent-keys';
import { usePersistedState } from '../lib/use-persisted-state';
import { captureOptions, savedItems, } from './myspace/data';
const noteTones = {
...
export default function MyspaceScreen({ onOpenDrawer, avatarLabel }) {
    ...
    const [stagedCapture, setStagedCapture] = useState(null);
    ...
    return (<SafeAreaView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {searching ? (<View style={styles.searchStateCard}>
          ...
          </View>) : queryActive ? (<View style={styles.searchStateCard}>
        ...
        {!queryActive && suggestions.length ? (<View style={styles.suggestionRow}>
            {suggestions.map((suggestion) => (<Pressable key={suggestion} style={styles.suggestionChip} onPress={() => setQuery(suggestion)}>
                <Ionicons name="sparkles-outline" size={13} color={theme.colors.accentStrong}/>
                <Text style={styles.suggestionChipText}>{suggestion}</Text>
              </Pressable>))}
          </View>) : null}

        {stagedCapture ? (<View style={styles.capturePreviewCard}>
            ...
            {captureDraft ? (<View style={styles.capturePreviewChips}>
                ...
                {(captureDraft.tags || '')
                    ...
                    .filter(Boolean)
                    .slice(0, 3)
                    .map((tag) => (<MetaChip key={tag} label={tag}/>))}
              </View>) : null}
            <View style={styles.capturePreviewActions}>
              ...
              <Pressable style={styles.previewActionFilled} onPress={() => {
        ...
        {!emptySearch ? (<>
            ...
            {recentTodayItems.length ? (<View style={styles.section}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pinnedRow}>
                  {recentTodayItems.map((item) => (<PinnedCard key={`today-${item.id}`} item={item} onPress={() => setSelectedItem(item)}/>))}
                </ScrollView>
              </View>) : null}
            ...
            <View style={styles.section}>
              ...
              {otherItems.length ? (<View style={styles.masonry}>
                  <View style={styles.column}>
                    {columns[0].map((item) => (<NoteCard key={item.id} item={item} query={query} match={matchMap.get(item.id) ?? null} onPress={() => setSelectedItem(item)}/>))}
                  </View>
                  <View style={styles.column}>
      ...
      <CaptureComposerSheet open={composerOpen} option={stagedCapture} draft={captureDraft} onClose={() => setComposerOpen(false)} onChangeDraft={(nextDraft) => setCaptureDraft(nextDraft)} onContinue={() => {
      ...
      <DetailSheet item={selectedItem} query={query} match={selectedItem ? matchMap.get(selectedItem.id) ?? null : null} onClose={() => setSelectedItem(null)} onAction={async (action, item) => {
            if (action === 'Pin') {
...
function PinnedCard({ item, onPress }) {
    ...
    return (<Pressable onPress={onPress} style={[styles.pinnedCard, { backgroundColor: tone.backgroundColor }]}>
...
function NoteCard({ item, query, match, onPress, }) {
    const tone = noteTones[item.accent];
    return (<Pressable onPress={onPress} style={[styles.noteCard, { backgroundColor: tone.backgroundColor }]}>
      <View style={[styles.notePin, { backgroundColor: tone.pin }]}/>
...
function AddSheet({ open, onClose, onSelectOption, }) {
        ...
        <SafeAreaView style={styles.sheetSafeArea}>
          <View style={styles.sheet}>
            ...
            <View style={styles.sheetOptions}>
              {captureOptions.map((option) => (<CaptureRow key={option.id} option={option} onPress={() => onSelectOption(option)}/>))}
            </View>
...
function CaptureComposerSheet({ open, option, draft, onClose, onChangeDraft, onContinue, }) {
    if (!option || !draft) {
        return null;
    }
      ...
      <View style={styles.modalBackdrop}>
        ...
        <SafeAreaView style={styles.sheetSafeArea}>
          <View style={styles.detailSheet}>
            ...
            <TextInput value={draft.title} onChangeText={(value) => onChangeDraft({ ...draft, title: value })} placeholder="Title" placeholderTextColor={theme.colors.textMuted} style={styles.composerField}/>
            <TextInput value={draft.body} onChangeText={(value) => onChangeDraft({ ...draft, body: value })} placeholder="What should future-you remember?" placeholderTextColor={theme.colors.textMuted} style={[styles.composerField, styles.composerFieldLarge]} multiline/>
            <TextInput value={draft.subject} onChangeText={(value) => onChangeDraft({ ...draft, subject: value })} placeholder="Subject or bucket" placeholderTextColor={theme.colors.textMuted} style={styles.composerField}/>
            <TextInput value={draft.source} onChangeText={(value) => onChangeDraft({ ...draft, source: value })} placeholder="Source" placeholderTextColor={theme.colors.textMuted} style={styles.composerField}/>
            <TextInput value={draft.tags} onChangeText={(value) => onChangeDraft({ ...draft, tags: value })} placeholder="Tags, comma separated" placeholderTextColor={theme.colors.textMuted} style={styles.composerField}/>
            <TextInput value={draft.ocrText} onChangeText={(value) => onChangeDraft({ ...draft, ocrText: value })} placeholder="OCR text or remembered phrase" placeholderTextColor={theme.colors.textMuted} style={[styles.composerField, styles.composerFieldLarge]} multiline/>
...
function CaptureRow({ option, onPress }) {
    return (<Pressable style={styles.captureRow} onPress={onPress}>
...
function DetailSheet({ item, query, match, onClose, onAction, }) {
    if (!item) {
        return null;
    }
        ...
        <SafeAreaView style={styles.sheetSafeArea}>
          <View style={styles.detailSheet}>
            ...
            <View style={styles.detailActions}>
              ...
              <Pressable style={[styles.detailAction, styles.detailActionFilled]} onPress={() => void onAction('Share', item)}>
...
function MetaChip({ label }) {
    return (<View style={styles.metaChip}>
...
function symbolToIcon(symbol) {
    ...
    if (symbol === 'rectangle.on.rectangle')
        return 'copy-outline';
