...
export function searchSavedItems(items, query, subject) {
    ...
    const scored = items
        ...
        for (const token of queryTokens) {
            ...
            if (item.body.toLowerCase().includes(token))
                score += 14;
            if (item.subject.toLowerCase().includes(token))
                score += 18;
            if (item.source.toLowerCase().includes(token))
                score += 10;
            if (item.dateLabel.toLowerCase().includes(token))
                score += 8;
            if ((item.ocrText ?? '').toLowerCase().includes(token))
                score += 16;
